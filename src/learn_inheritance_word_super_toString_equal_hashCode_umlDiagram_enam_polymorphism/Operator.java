...
public class Operator extends Employee {
    ...
    public boolean equals(Object o) {
        ...
        if (this.taxiOrders == op.taxiOrders && this.countOrder == op.countOrder
                ...
                && this.getId() == op.getId() && this.getSalary() == op.getSalary() &&
                this.getHireDate() == op.getHireDate()
                && this.getPhoneNumber() == op.getPhoneNumber() && this.getName() == op.getName() &&
                this.getSurName() == op.getSurName()) {
            return true;
        }

        if (this.taxiOrders != op.taxiOrders || this.countOrder != op.countOrder ||
                this.workPlaceNumber != op.workPlaceNumber ||
                this.dateOfOrder != op.dateOfOrder || this.getId() != op.getId() ||
                this.getSalary() != op.getSalary()
                || this.getHireDate() != op.getHireDate()) {
            return false;
        } else {
            return false;
